...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if ((a.GetTag() == 1 && b.GetTag() == 2) || (a.GetTag() == 2 && b.GetTag() == 1)) {
		if (a.GetTag() == 2) {
			...
			gameWorld.SetObjectCount((gameWorld.GetObjectCount() - 1));
			std::fstream my_file;
			...
			if (b.getScore() > x) {
				std::ofstream f(Assets::DATADIR + "Highscore.txt");
				f << b.getScore() << std::endl;
				f.close();
			...
			if (a.getScore() > x) {
				std::ofstream f(Assets::DATADIR + "Highscore.txt");
				f << a.getScore() << std::endl;
				f.close();

			}
		}
	}
	if ((a.GetTag() == 5 && b.GetTag() == 2) || (a.GetTag() == 2 && b.GetTag() == 5)) {
		if (b.GetTag() == 2) {
			b.setActive(false);
			...
			if (gameWorld.GetPlayer()->getScore() > x) {
				std::ofstream f(Assets::DATADIR + "Highscore.txt");
				f << gameWorld.GetPlayer()->getScore() << std::endl;
				f.close();
	...
	if ((a.GetTag() == 1 && b.GetTag() == 3) || (a.GetTag() == 3 && b.GetTag() == 1)) {
		//std::cout << "contact";
		if (a.GetTag() == 1) {
			b.GetAssociated()->setActive(false);
			b.GetAssociated()->SetBoundingVolume(nullptr);
			//std::cout << "contactA";
		}
		if (b.GetTag() == 1) {
			a.GetAssociated()->setActive(false);
			a.GetAssociated()->SetBoundingVolume(nullptr);
			//std::cout << "contactB";
		}
	}
	...
	float ja = (-(1.0f + physA->GetRestitution()) * impulseForce) /
		(totalMass + angularEffect);
	float jb = (-(1.0f + physB->GetRestitution()) * impulseForce) /
		(totalMass + angularEffect);
