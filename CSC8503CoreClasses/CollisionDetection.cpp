...
bool CollisionDetection::RayCapsuleIntersection(const Ray& r, const Transform& worldTransform, const CapsuleVolume& volume, RayCollision& collision) {
	...
	float halflength = volume.GetHalfHeight() - volume.GetRadius();
	...
	Vector3 point = r.GetPosition() + (r.GetDirection() * capsuleProj);
	Vector3 capsuleTop = worldTransform.GetPosition() + (worldTransform.GetOrientation() * Vector3(0, halflength, 0));
	Vector3 capsuleBottom = worldTransform.GetPosition() - (worldTransform.GetOrientation() * Vector3(0, halflength, 0));
	//Debug::DrawLine(capsuleTop, capsuleBottom, Vector4(0, 0, 1, 1));
	Vector3 p = ClosestPointOnLine(capsuleTop, capsuleBottom, point);

	float capsuleDist = (point - p).Length();

	if (capsuleDist > radius) {
		return false;
	}

	float offset =
		sqrt((radius * radius) - (capsuleDist * capsuleDist));

	collision.rayDistance = capsuleProj - (offset);
	collision.collidedAt = r.GetPosition() +
		(r.GetDirection() * collision.rayDistance);
	return true;
}
...
bool CollisionDetection::CapsuleIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA,
	...
	float halflengthA = volumeA.GetHalfHeight() - volumeA.GetRadius();
	Vector3 capsuleTopA = worldTransformA.GetPosition() + (worldTransformA.GetOrientation() * Vector3(0, halflengthA, 0));
	Vector3 capsuleBottomA = worldTransformA.GetPosition() - (worldTransformA.GetOrientation() * Vector3(0, halflengthA, 0));
	//Capsule B
	float radiusB = volumeB.GetRadius();
	float halflengthB = volumeB.GetHalfHeight() - volumeB.GetRadius();
	Vector3 capsuleTopB = worldTransformB.GetPosition() + (worldTransformB.GetOrientation() * Vector3(0, halflengthB, 0));
	Vector3 capsuleBottomB = worldTransformB.GetPosition() - (worldTransformB.GetOrientation() * Vector3(0, halflengthB, 0));

	Vector3 v1 = capsuleBottomB - capsuleBottomA;
	Vector3 v2 = capsuleTopB- capsuleBottomA;
	Vector3 v3 = capsuleBottomB- capsuleTopA;
	Vector3 v4 = capsuleTopB- capsuleTopA;

	float d0 = Vector3::Dot(v1, v1);
	float d1 = Vector3::Dot(v2, v2);
	float d2 = Vector3::Dot(v3, v3);
	float d3 = Vector3::Dot(v4, v4);
	...
	Vector3 bestB = ClosestPointOnLine(capsuleBottomB, capsuleTopB, bestA);

	bestA = ClosestPointOnLine(capsuleBottomA, capsuleTopA, bestB);
...
bool CollisionDetection::AABBCapsuleIntersection(
	...
	float radiusA = volumeA.GetRadius();
	float halflength = volumeA.GetHalfHeight() - volumeA.GetRadius();

	Vector3 capsuleTop = worldTransformA.GetPosition() + (worldTransformA.GetOrientation() * Vector3(0, halflength, 0));
	Vector3 capsuleBottom = worldTransformA.GetPosition() - (worldTransformA.GetOrientation() * Vector3(0, halflength, 0));
	//Debug::DrawLine(capsuleTop, capsuleBottom, Vector4(0, 0, 1, 1));
	Vector3 p = ClosestPointOnLine(capsuleTop, capsuleBottom, worldTransformB.GetPosition());

	Vector3 localPoint = (worldTransformB.GetPosition() - p) - closestPointOnBox;
	float distance = localPoint.Length();
...
Vector3 CollisionDetection::ClosestPointOnLine(Vector3 A, Vector3 B, Vector3 Point)
{
	Vector3 AB = B - A;
	float t = Maths::Clamp((Vector3::Dot(Point - A, AB) / Vector3::Dot(AB, AB)),0.0f,1.0f);
	return A + Vector3(AB.x * t, AB.y * t, AB.z * t);
}
...
bool CollisionDetection::SphereCapsuleIntersection(
	...
	Vector3 capsuleBottom = worldTransformA.GetPosition() - (worldTransformA.GetOrientation() * Vector3(0, halflength, 0));
	//Debug::DrawLine(capsuleTop, capsuleBottom,Vector4(0,0,1,1));
	Debug::DrawLine(worldTransformA.GetPosition(), capsuleBottom, Vector4(1, 0, 0, 0));
	Debug::DrawLine(worldTransformA.GetPosition(), capsuleTop, Vector4(1, 1, 1, 1));
	Debug::DrawLine(worldTransformA.GetPosition(), worldTransformA.GetPosition() + Vector3(radiusA,0,0), Vector4(1, 0, 0, 0));
	Debug::DrawLine(worldTransformA.GetPosition(), worldTransformA.GetPosition() - Vector3(radiusA, 0, 0), Vector4(1, 0, 0, 0));
	Vector3 p = ClosestPointOnLine(capsuleTop, capsuleBottom, worldTransformB.GetPosition());

	Vector3 delta = worldTransformB.GetPosition() - p;
	float radii = volumeA.GetRadius() + volumeB.GetRadius();
	float deltaLength = delta.Length();
