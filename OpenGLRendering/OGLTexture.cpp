...
TextureBase* OGLTexture::SRGBTextureFromFilename(const std::string& name) {
	char* texData = nullptr;
	int width = 0;
	int height = 0;
	int channels = 0;
	...
	OGLTexture* gltex = new OGLTexture();
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB, width, height, 0, sourceType, GL_UNSIGNED_BYTE, texData);
