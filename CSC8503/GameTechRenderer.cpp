...
void GameTechRenderer::RenderCircle(float cx, float cy, float r, const Vector4 &color)
{
	OGLShader shader = OGLShader("circle.vert", "circle.frag");
	BindShader(&shader);
	const auto &u_center = glGetUniformLocation(shader.GetProgramID(), "u_Center");
	const auto &u_color = glGetUniformLocation(shader.GetProgramID(), "u_Color");
	const auto &u_radius = glGetUniformLocation(shader.GetProgramID(), "u_Radius");
...
void GameTechRenderer::RenderTriangle(Vector2 &v1, Vector2 &v2, Vector2 &v3, Vector4 &color)
{
	OGLShader shader = OGLShader("triangle.vert", "triangle.frag");
	BindShader(&shader);
	const auto &u_color = glGetUniformLocation(shader.GetProgramID(), "u_Color");
...
void GameTechRenderer::RenderMap()
{
	...
	gameWorld.OperateOnContents(
		[&](GameObject *o)
		{
			if (o->GetName() != "cube" && o->GetName() != "goose")
				return;
			Vector2 p_pos(player->GetTransform().GetPosition().x, player->GetTransform().GetPosition().z);
			Vector2 o_pos(o->GetTransform().GetPosition().x, o->GetTransform().GetPosition().z);
	...
	{
		Vector2 p_pos(player->GetTransform().GetPosition().x, player->GetTransform().GetPosition().z);
		Vector2 o_pos(object->GetTransform().GetPosition().x, object->GetTransform().GetPosition().z);
...
void GameTechRenderer::RenderRectangle(float px, float py, float width, float height, Vector4 &color)
{
...
void GameTechRenderer::handlePause()
{

	if (gameWorld.isPausing())
	{
		auto color = Debug::BLACK;
		float width = 400;
		float height = 200;
		RenderRectangle(
			windowWidth / 2.0f - width / 2.0f,
			windowHeight / 2.0f + height / 2.0f,
			width,
			height,
			color);
		Debug::Print("pausing", Vector2(50, 50), Debug::RED);
	}
}
